...
def _ensure_classifier_models(classifier_dir):
    features_path = os.path.join(classifier_dir, "model_features.joblib")
    if not os.path.exists(features_path):
        try:
            from classifier.generate_models import generate_models
            generate_models()
        except Exception as e:
            pass
...
def ml_inference_node(state: ClaimGraphState):
    ...
    try:
        ...
        _ensure_classifier_models(classifier_dir)

        features_path = os.path.join(classifier_dir, "model_features.joblib")
        if not os.path.exists(features_path):
            features_path = os.path.join("classifier", "model_features.joblib")
        ...
        try:
            clf_path = os.path.join(classifier_dir, "approval_classifier.joblib")
        ...
        try:
            reg_path = os.path.join(classifier_dir, "payout_regressor.joblib")
